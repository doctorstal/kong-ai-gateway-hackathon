...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  return (
    <div className="h-full flex flex-col bg-base-200 w-64 min-w-64 border-r">
      <div className="p-4">
        <button className="btn btn-primary w-full" onClick={onNewChat}>
          <svg
      ...
      <div className="flex-1 overflow-y-auto">
        {chats.length === 0 ? (
          <div className="text-center text-gray-500 p-4">No chat history</div>
        ) : (
          <ul className="menu p-2 gap-1">
            {chats.map((chat) => (
              <li
                key={chat.id}
                className={`grid grid-cols-[1fr_min-content] items-center ${
              ...
              >
                <div
                  className="px-3 py-2 whitespace-nowrap overflow-hidden text-allipsis cursor-pointer"
                  style={{ display: "block" }}
